...
public interface Action {
    ...
    void sendTitle(String action, UUID uuid, String command, String title, String subTitle, int fadeIn, int stay,
            int fadeOut);
    ...
    default void alert(UUID uuid, String playerName, String serverName, String command) {
        ...
        final List<String> notificationMessage = MessageTranslator.replaceMessageList(
                Storage.ConfigSections.Messages.NOTIFICATION.ALERT, "%player%", playerName, "%command%", displayCommand,
                "%server%", serverName);
