...
public class ResponseHandler {
    ...
    public static void update() {
        ...
        Storage.Files.CUSTOM_RESPONSES.getKeys(true).stream().filter(key -> !key.equals("default-message"))
                .forEach(key ->
                {
    ...
    public static String replaceArgsVariables(String input, String command) {
        ...
        try {
            ...
            Logger.warning(
                    "Failed to parse and replace %args[...]% parts. Please make sure that the format is correct and that befitting the written numbers.");
    ...
    public static List<String> getResponse(UUID uuid, String playerName, String serverName, String input) {
        ...
        return getResponse(uuid, playerName, serverName, input,
                Storage.ConfigSections.Settings.CANCEL_COMMAND.SUB_COMMAND_RESPONSE.getLines());
            ...
            List<String> defaultMessage)
    {
        ...
        final String finalInput = input.startsWith("/") && input.length() > 1 ? input.substring(1) : input;

        final Optional<Response> optionalResponse = RESPONSES.stream().filter(response -> {

            for (String trigger : response.triggers)
                if (trigger.endsWith("*") ? finalInput.startsWith(StringUtils.replace(trigger, "*", ""))
                        : finalInput.equals(trigger))
                    return true;
        ...
        if (uuid != null && optionalResponse.isPresent()) {
            ...
            return new ArrayList<>(optionalResponse.get().message).stream()
                    .map(s -> replaceArgsVariables(s, finalInput)).collect(Collectors.toList());
    ...
    private static class Response {
        ...
        public Response(final String key) {
            ...
            this.message = (List<String>) Storage.Files.CUSTOM_RESPONSES.getOrSet(key + ".message",
                    SubArguments.BLOCKED_MESSAGE);
                ...
                switch (split[0].toLowerCase()) {
                    ...
                    case "effect":
                        ...
                        try {
                            ...
                            Logger.warning("! Failed to read action: " + action);
                            Logger.warning(
                                    "  > Duration " + split[2].toUpperCase() + " is not a valid integer! (e.g: 1)");
                            continue;
                        ...
                        try {
                            ...
                            Logger.warning("! Failed to read action: " + action);
                            Logger.warning(
                                    "  > Amplifier " + split[3].toUpperCase() + " is not a valid integer! (e.g: 1)");
                            continue;
                    ...
                    case "title":
                        ...
                        try {
                            ...
                            Logger.warning("! Failed to read action: " + action);
                            Logger.warning(
                                    "  > Fade-In " + split[3].toUpperCase() + " is not a valid integer! (e.g: 300)");
                            continue;
                        ...
                        try {
                            ...
                            Logger.warning("! Failed to read action: " + action);
                            Logger.warning(
                                    "  > Stay " + split[4].toUpperCase() + " is not a valid integer! (e.g: 3000)");
                            continue;
                        ...
                        try {
                            ...
                            Logger.warning("! Failed to read action: " + action);
                            Logger.warning(
                                    "  > Fade-Out " + split[5].toUpperCase() + " is not a valid integer! (e.g: 300)");
                            continue;
                    ...
                    case "sound":
                        if (split.length != 4) {
                            ...
                            Logger.warning(
                                    "  > e.g: sound::" + (Reflection.isProxyServer() ? "entity.ender_dragon.growl:"
                                            : "ENTITY_ENDER_DRAGON_GROWL") + "::1.0::1.0");
                            continue;
                        ...
                        try {
                            ...
                            if (volume < 0 || volume > 10) {
                                ...
                                Logger.warning("! Failed to read action: " + action);
                                Logger.warning("  > Your volume (" + split[2].toUpperCase()
                                        + ") must be between 0.0 and 10.0! (e.g: 1.0)");
                                continue;
                            ...
                            Logger.warning("! Failed to read action: " + action);
                            Logger.warning(
                                    "  > Volume " + split[2].toUpperCase() + " is not a valid float! (e.g: 1.0f)");
                            continue;
                        ...
                        try {
                            ...
                            if (pitch < 0 || pitch > 10) {
                                ...
                                Logger.warning("! Failed to read action: " + action);
                                Logger.warning("  > Your pitch (" + split[2].toUpperCase()
                                        + ") must be between 0.0 and 2.0! (e.g: 1.0)");
                                continue;
                        ...
                        ActionHandler.playSound(action, uuid,
                                Reflection.isProxyServer() ? split[1].toLowerCase() : split[1].toUpperCase(), volume,
                                pitch);
                    ...
                    case "p2b-message":
                        ...
                        Communicator.Proxy2Backend.sendConsoleMessage(serverName, ResponseHandler
                                .replaceArgsVariables(StringUtils.replace(split[1], "%player%", playerName), command));
                        continue;
                    ...
                    case "p2b-execute":
                        ...
                        Communicator.Proxy2Backend.sendExecutePlayerCommand(serverName, uuid, ResponseHandler
                                .replaceArgsVariables(StringUtils.replace(split[1], "%player%", playerName), command));
                    ...
                    case "p2b-console":
                        ...
                        Communicator.Proxy2Backend.sendExecuteConsoleCommand(serverName, ResponseHandler
                                .replaceArgsVariables(StringUtils.replace(split[1], "%player%", playerName), command));
