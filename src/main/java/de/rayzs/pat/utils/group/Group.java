...
public class Group implements Serializable {
    ...
    private final ExpireCache<String, List<GroupBlacklist>> cachedServerGroupBlacklists = new ExpireCache<>(1,
            TimeUnit.HOURS);
    ...
    public String getGroupInfo() {
        ...
        builder.append("  commands: ").append(String.join(", ", getCommands())).append("\n");
    ...
    private void loadServerBlacklist() {
        ...
        Storage.Files.STORAGE.getKeys("groups." + groupName + ".servers", false)
    ...
    public List<String> getBlacklistServerNames(String server) {
        ...
        return groupServerBlacklist.keySet().stream().filter(key -> Storage.isServer(key, server)).toList();
    ...
    public List<GroupBlacklist> getAllServerGroupBlacklist(String server, boolean useDefault) {
        ...
        if (useDefault)
            groupBlacklists.add(generalGroupBlacklist);

        GroupBlacklist groupBlacklist;

        for (String key : groupServerBlacklist.keySet()) {

            if (!Storage.isServer(key, server))
                continue;
