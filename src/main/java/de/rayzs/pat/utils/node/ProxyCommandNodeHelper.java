...
public class ProxyCommandNodeHelper<T> {
    ...
    public static CommandNode createDummyCommandNode(String command) {
        ...
        return LiteralArgumentBuilder.literal(command).executes(DUMMY_COMMAND)
                .then(RequiredArgumentBuilder.argument("args", (ArgumentType) StringArgumentType.greedyString())
                        .suggests(DEFAULT_SUGGESTION_PROVIDER).executes(DUMMY_COMMAND))
                .build();
    ...
    public static CommandNode createEmptyDummyCommandNode(String command) {
        ...
        return LiteralArgumentBuilder
                .literal(command).executes(DUMMY_COMMAND).then(RequiredArgumentBuilder
                        .argument("args", (ArgumentType) StringArgumentType.greedyString()).executes(DUMMY_COMMAND))
                .build();
        ...
        if (childNode == null) {
            ...
            if (!autoSuggestions || cancel || !stop && index != args.length - 1) {
                ...
                newNode = LiteralArgumentBuilder.literal(part)
                        .then(RequiredArgumentBuilder.argument("args", StringArgumentType.string())).build();
