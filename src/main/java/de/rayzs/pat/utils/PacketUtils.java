...
public class PacketUtils {
     ...
     * /md_5/bungee/protocol/DefinedPacket.java#L307-L327
     */
     ...
     * source:
     * https://github.com/SpigotMC/BungeeCord/blob/master/protocol/src/main/java/net
     * /md_5/bungee/protocol/DefinedPacket.java#L88-L108
     */
    ...
    public static void writeString(String string, ByteBuf byteBuf, int maxLength) throws Exception {
        ...
        if (string.length() > maxLength)
            throw new Exception(
                    "Cannot send string longer than " + maxLength + " (got " + string.length() + " characters)");

        byte[] bytes = string.getBytes(StandardCharsets.UTF_8);
        if (bytes.length > maxLength * 3)
            throw new Exception(
                    "Cannot send string longer than " + (maxLength * 3) + " (got " + bytes.length + " bytes)");
