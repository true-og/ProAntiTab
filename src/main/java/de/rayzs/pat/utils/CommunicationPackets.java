...
public class CommunicationPackets {
    ...
    public static byte[] preparePacket(Object object, UUID id) {
        ...
        try (ObjectOutputStream outputStream = new ObjectOutputStream(arrayOutputStream)) {
            ...
            final byte[] encryptedBytes = xor(arrayOutputStream.toByteArray(),
                    isInitialPacket ? Storage.TOKEN : id.toString());
    ...
    public static Object readPacket(byte[] bytes, UUID id) {
        ...
        bytes = xor(bytes, isInitialPacket(init) ? Storage.TOKEN : id.toString());
    ...
    public static class Proxy2Backend {
        ...
        public record DataSyncPacket(Messages messages, AutoLowerCase autoLowerCase, UnknownCommand unknownCommand)
