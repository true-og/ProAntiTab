...
public class Logger {
    ...
    static {
        ...
        final String software = Reflection.getSoftware().isProxySoftware() ? Reflection.getSoftware().toString()
    ...
    private final static LoggerTemplate LOGGER = Reflection.isVelocityServer() ? new VelocityLogger()
            : Reflection.isProxyServer() ? new BungeeLogger() : new BukkitLogger();
    ...
    private static String createInfo() {
        ...
        if (!Reflection.isProxyServer()) {
            ...
            builder.append("Detected Software-API: ").append(Reflection.getSoftware().toString()).append("\n\n");

        }

        builder.append("Updated: ").append(!Storage.OUTDATED).append("\n\n");
        ...
        builder.append("Installed plugins: \n  ")
                .append(String.join("\n  ", Storage.getLoader().getPluginNames("%n (%v)"))).append("\n\n");

        builder.append("Detected supported plugins:").append("\n");

        builder.append(" LuckPerms: ").append(Storage.getPermissionPlugin() == PermissionPlugin.LUCKPERMS).append("\n");

        if (!Reflection.isProxyServer()) {

            builder.append(" GroupManager: ").append(Storage.getPermissionPlugin() == PermissionPlugin.GROUPMANAGER)
                    .append("\n");

        }

        builder.append(" PlaceholderAPI: ").append(Storage.USE_PLACEHOLDERAPI).append("\n");

        builder.append(" ViaVersion: ").append(Storage.USE_VIAVERSION).append("\n\n");

        if (!Reflection.isProxyServer()) {

            builder.append("server-name: ")
                    .append(Storage.ConfigSections.Settings.HANDLE_THROUGH_PROXY.ENABLED ? Storage.SERVER_NAME
                            : "Not connected to a proxy using PAT")
                    .append("\n");

            builder.append("last-received-sync: ")
                    .append(Storage.ConfigSections.Settings.HANDLE_THROUGH_PROXY.ENABLED
        ...
        builder.append("\ncommands: ").append(String.join(", ", Storage.Blacklist.getBlacklist().getCommands()))
                .append("\n\n");

        if (Reflection.isProxyServer()) {

            builder.append("\nservers: ").append(String.join(", ", Storage.getServers())).append("\n");

            builder.append("last-sent-sync: ")
                    .append(Storage.ConfigSections.Settings.DISABLE_SYNC.DISABLED ? "Deactivated"
                            : (TimeConverter.calcAndGetTime(Communicator.get().getLastSync())))
                    .append("\n\n");
            ...
            while (iterator.hasNext()) {
                ...
                builder.append(clientInfo.getServerName()).append(" (")
                        .append(clientInfo.getFormattedLastReceivedKeepAlivePacketTime() + " / "
                                + clientInfo.getFormattedSyncTime())
                        .append(")");
        ...
        if (Reflection.isProxyServer()) {
            ...
            for (Map.Entry<String, GeneralBlacklist> blacklist : Storage.Blacklist.getBlacklists()) {
                ...
                builder.append("  ").append(blacklist.getKey()).append(": ")
                        .append(String.join(", ", blacklist.getValue().getCommands())).append("\n");
