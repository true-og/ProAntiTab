...
public class ExtractCommand extends ProCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (!all && !pluginFound) {
            ...
            sender.sendMessage(StringUtils.replace(Storage.ConfigSections.Messages.EXTRACT.PLUGIN_NOT_FOUND, "%plugin%",
                    pluginName));
        ...
        List<String> commands = all ? Storage.getLoader().getAllCommands(mode.doesIncludesColons())
        ...
        BlacklistStorage storage = group == null ? Storage.Blacklist.getBlacklist() : group.getGeneralGroupBlacklist();
        ...
        sender.sendMessage(StringUtils.replace(Storage.ConfigSections.Messages.EXTRACT.SUCCESS, "%amount%",
                String.valueOf(commands.size()), "%plugin%", pluginName));
