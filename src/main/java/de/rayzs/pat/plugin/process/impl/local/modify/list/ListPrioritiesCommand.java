...
public class ListPrioritiesCommand extends ProCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        String groupsPriorityListMessage = buildGroupPriorityMessage(GroupManager.getGroups(),
                Storage.ConfigSections.Messages.GROUP.LIST_PRIORITY_SPLITTER,
                Storage.ConfigSections.Messages.GROUP.LIST_PRIORITY_GROUPS);

        String message = StringUtils.replace(groupsPriorityListMessage, "%size%",
                String.valueOf(GroupManager.getGroups().size()), "%groups%", groupsPriorityListMessage);
    ...
    private String buildGroupPriorityMessage(List<Group> list, String splitter, String format) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            builder.append(format.replace("%group%", group.getGroupName()).replace("%priority%",
                    String.valueOf(group.getPriority())));
            if (!end && splitter != null)
