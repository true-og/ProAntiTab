...
public class StatsCommand extends ProCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        while (iterator.hasNext()) {
            ...
            statsBuilder.append(StringUtils.replace(Storage.ConfigSections.Messages.STATS.SERVER, "%updated%",
                    client.getFormattedSyncTime(), "%servername%", client.getServerName(), "%last_alive_response%",
                    client.getFormattedLastReceivedKeepAlivePacketTime()));
        ...
        Storage.ConfigSections.Messages.STATS.STATISTIC.getLines().forEach(message -> {
            ...
            message = StringUtils.replace(message, "%server_count%",
                    String.valueOf(Communicator.get().getClients().size()), "%last_sync_time%",
                    TimeConverter.calcAndGetTime(Communicator.get().getLastSync()), "%last_alive_response_time%",
                    TimeConverter.calcAndGetTime(Communicator.get().getLastReceivedKeepAliveResponse()), "%servers%",
                    found ? statsBuilder.toString() : Storage.ConfigSections.Messages.STATS.NO_SERVER);
