...
public class BungeePingListener implements Listener {
    ...
    public void onProxyPing(ProxyPingEvent event) {
        ...
        int online = proxyServer.getOnlineCount(),
                onlineExtend = online + Storage.ConfigSections.Settings.CUSTOM_PROTOCOL_PING.EXTEND_COUNT, max = -1;
        ...
        try {
            ...
            Logger.warning(
                    "Failed to read max-players count for %max% placeholder! Using -1 as default value instead.");

        }

        ServerPing serverPing = event.getResponse();

        int protocol = Storage.ConfigSections.Settings.CUSTOM_PROTOCOL_PING.ALWAYS_SHOW ? -1
                : serverPing.getVersion().getProtocol();
        String version = replaceString(Storage.ConfigSections.Settings.CUSTOM_PROTOCOL_PING.PROTOCOL, online,
                onlineExtend, max);
    ...
    private String replaceString(String string, int online, int onlineExtend, int max) {
        ...
        return string.replace("&", "§").replace("%online_extended%", String.valueOf(onlineExtend))
