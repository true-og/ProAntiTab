...
public class VelocityAntiTabListener {
    ...
    public void onTabComplete(TabCompleteEvent event) {
        ...
        if (PermissionUtil.hasBypassPermission(sender) || event.getSuggestions().isEmpty()
    ...
    public void onPlayerAvailableCommands(PlayerAvailableCommandsEvent event) {
        ...
        try {
            ...
            event.getRootNode().getChildren().stream().filter(command -> command != null && command.getName() != null)
            ...
            final boolean newer = player.getProtocolVersion().getProtocol() > 340,
                    argsChildrenExist = event.getRootNode().getChild("args") != null;
            ...
            event.getRootNode().getChildren().removeIf(command -> {
                ...
                if (Storage.ConfigSections.Settings.CUSTOM_PLUGIN.isTabCompletable(command.getName())
                        || Storage.ConfigSections.Settings.CUSTOM_VERSION.isTabCompletable(command.getName()))
                {
