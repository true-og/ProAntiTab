...
public class PlHidePro extends Converter {
    ...
    public void apply(CommandSender sender) {
        ...
        Storage.ConfigSections.Settings.UPDATE_GROUPS_PER_WORLD.getConfig().setAndSave("update-groups-per-world",
                updateGroupPerWorld);

        boolean autoLowercaseCommands = (boolean) config.get("auto_lowercase_base_commands");
        Storage.ConfigSections.Settings.AUTO_LOWERCASE_COMMANDS.getConfig()
                .setAndSave("auto-lowercase-commands.enabled", autoLowercaseCommands);
        ...
        if (f3brand) {
            ...
            Storage.ConfigSections.Settings.CUSTOM_BRAND.getConfig().setAndSave("custom-server-brand.brands",
                    new ArrayList<>(Collections.singletonList(f3brandString.replace("§", "&"))));
