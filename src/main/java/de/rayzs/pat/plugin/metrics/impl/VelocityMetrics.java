...
public class VelocityMetrics {
    ...
    public static class Factory {
         ...
         *                  plugin id?</a>
         *                  <p>
         *                  Not to be confused with Velocity's
         *                  {@link PluginDescription#getId()} method!
         * @return A Metrics instance that can be used to register custom charts.
         *         <p>
         *         The return value can be ignored, when you do not want to register
         *         custom charts.
         */
    ...
    private VelocityMetrics(Object plugin, ProxyServer server, Path dataDirectory, int serviceId) {
        ...
        metricsBase = new MetricsBase("velocity", config.getServerUUID(), serviceId, config.isEnabled(),
                this::appendPlatformData, this::appendServiceData,
                task -> server.getScheduler().buildTask(plugin, task).schedule(), () -> true,
                config.isLogErrorsEnabled(), config.isLogSentDataEnabled(), config.isLogResponseStatusTextEnabled());
    ...
    public static class MetricsBase {
        ...
        public MetricsBase(String platform, String serverUuid, int serviceId, boolean enabled,
                ...
                Supplier<Boolean> checkServiceEnabledSupplier, boolean logErrors, boolean logSentData,
        ...
        private void submitData() {
            ...
            JsonObjectBuilder.JsonObject[] chartData = customCharts.stream()
                    .map(customChart -> customChart.getRequestJsonObject()).filter(Objects::nonNull)
    ...
    public static class MetricsConfig {
        ...
        private void writeConfig() throws IOException {
            ...
            configContent
                    .add("# how many people use their plugin and their total player count. It's recommended to keep");
            configContent
                    .add("# bStats enabled, but if you're not comfortable with this, you can turn this setting off.");
            configContent
                    .add("# There is no performance penalty associated with having metrics enabled, and data sent to");
        ...
        private Optional<String> getConfigValue(String key, List<String> lines) {
            ...
            return lines.stream().filter(line -> line.startsWith(key + "="))
                    .map(line -> line.replaceFirst(Pattern.quote(key + "="), "")).findFirst();
