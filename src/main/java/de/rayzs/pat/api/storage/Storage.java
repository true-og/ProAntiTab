...
public class Storage {
    ...
    public static void handleChange(String server) {
        ...
        if (Reflection.isProxyServer()) {
            ...
            if (server != null) {
                ...
                List<String> associatedServers = getServers().stream()
                        .filter(s -> !s.equals(server) && isServer(server, s)).toList();
    ...
    public static class Files {
        ...
        public static final ConfigurationBuilder CONFIGURATION = Configurator.get("config"),
                STORAGE = Configurator.get("storage"), PLACEHOLDERS = Configurator.get("placeholders"),
                CUSTOM_RESPONSES = Configurator.get("custom-responses"), TOKEN = Configurator.get("token");
            ...
            public static String findAndReplace(Player player, String request) {
                ...
                if (USE_PLACEHOLDERAPI) {
                    ...
                    for (PlaceholderStorage storage : PLACEHOLDERS) {
                        ...
                        if (!storage.getRequest().startsWith(request))
                            continue;
    ...
    public static class Blacklist {
        ...
        private static final ExpireCache<String, List<GeneralBlacklist>> CACHED_SERVER_BLACKLIST = new ExpireCache<>(1,
                TimeUnit.HOURS);
        ...
        private static void loadCachedServerBlacklists(String server) {
            ...
            List<GeneralBlacklist> blacklists = SERVER_BLACKLISTS.entrySet().stream()
                    .filter(entry -> isServer(entry.getKey(), server)).map(Map.Entry::getValue).toList();
        ...
        public static boolean canPlayerAccess(CommandSender sender, List<Group> groups, String command,
                BlockType type)
        {
        ...
        public static boolean isBlocked(String command, BlockType type, String server) {
            ...
            boolean all = isListed("*", type, server), listed = isListed(command, type, server),
        ...
        public static class BlockTypeFetcher {
            ...
            public static String modify(String command, BlockType type) {
                ...
                if (type == null || type == BlockType.BOTH || command.length() <= type.toString().length()
                        || !command.startsWith(type.toString()))
                    return command;
