...
public class CustomProtocolPingSection extends ConfigStorage {
    ...
    public void load() {
        ...
        ALWAYS_SHOW = new ConfigSectionHelper<Boolean>(this, "always-show",
                Reflection.isProxyServer() || Reflection.isPaper()).getOrSet();
        USE_EXTEND_AS_MAX_COUNT = new ConfigSectionHelper<Boolean>(this, "use-as-maxplayers", false).getOrSet();
        PROTOCOL = new ConfigSectionHelper<String>(this, "protocol", "&f&lProAntiTab &7(&a%online%&7/&c%max%&7)")
                .getOrSet();
        ...
        PLAYERLIST = new MultipleMessagesHelper(this, "custom-playerlist.list",
                Arrays.asList("&8> &7Online players: &f%online%&7/&c%max%", "&8> &7Using &fProAntiTab"));

        if (HIDE_PLAYERS && USE_CUSTOM_PLAYERLIST) {

            Logger.warning(
                    "You can't use both 'hide-players' and 'custom-playerlist' at the same time! (Section: custom-protocol-ping)");
            Logger.warning("You gotta choose what feature from those two you want to use.");
            Logger.warning(
                    "In order to avoid any problems, the 'custom-playerlist' feature has been temporarily disabled.");
