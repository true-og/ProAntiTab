...
public class InfoSection extends ConfigStorage {
    ...
    public void load() {
        ...
        VERSION_OUTDATED = new ConfigSectionHelper<String>(this, "version.outdated", "&cOutdated (%newest_version%)")
                .getOrSet();
        SYNC_TIME = new ConfigSectionHelper<String>(this, "proxy-sync.time", "&e%time%").getOrSet();

        if (!Reflection.isProxyServer()) {

            SYNC_WAITING = new ConfigSectionHelper<String>(this, "proxy-sync.waiting",
                    "&cWaiting to establish connection...").getOrSet();
        ...
        MESSAGE = new MultipleMessagesHelper(this, "message",
                Arrays.asList("&7Necessary information about &fPAT&8:", "&7  Version: &e%current_version%",
                        "&7  Status: %version_status%", "&7  Last sync with proxy: &e%sync_time%",
                        "&7  Last received alive packet: %last_alive_response%", "&7  Proxy sync-token: &e%token%",
