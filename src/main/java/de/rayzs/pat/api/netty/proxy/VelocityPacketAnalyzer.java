...
public class VelocityPacketAnalyzer {
    ...
    public static boolean inject(Player player) {
        ...
        try {
            ...
            Object connectedPlayerObj = connectedPlayerConnectionClass.cast(player), minecraftConnectionObj = Reflection
                    .getMethodsByName(connectedPlayerConnectionClass, "getConnection").get(0)
                    .invoke(connectedPlayerObj);

            channel = (Channel) Reflection.getMethodsByName(minecraftConnectionClass, "getChannel").get(0)
                    .invoke(minecraftConnectionObj);
            ...
            channel.pipeline().addBefore(VelocityPacketAnalyzer.HANDLER_NAME, VelocityPacketAnalyzer.PIPELINE_NAME,
                    new PacketDecoder(player));
            VelocityPacketAnalyzer.INJECTED_PLAYERS.put(player, channel);

        } catch (NoSuchElementException e) {

            Logger.warning("Failed to find 'handler' inside player pipeline! (name=" + player.getUsername() + ", uuid="
                    + player.getUniqueId() + ", version=" + player.getProtocolVersion() + ", brand="
    ...
    private static void modifyCommands(Player player, CommandSender sender, AvailableCommandsPacket commands) {
        ...
        if (commands.getRootNode().getChildren().size() == 1 && player.getProtocolVersion().getProtocol() > 340
        ...
        final List<String> playerCommands = commandsCache.getPlayerCommands(commandsAsString, sender, groups,
                serverName);
        helper.removeIf(str -> {
    ...
    private static class PacketDecoder extends ChannelDuplexHandler {
            ...
            if (packet.getClass().getSimpleName().equals("SignedChatCommand")) {
                ...
                try {
                    ...
                    if (signedChatCommandPacketClass == null)
                        signedChatCommandPacketClass = Class
                                .forName("com.velocitypowered.proxy.crypto.SignedChatCommand");
            ...
            if (packet instanceof TabCompleteRequestPacket request) {
                ...
                if (request.getCommand() != null) {
                    ...
                    if (Storage.ConfigSections.Settings.PATCH_EXPLOITS.isMalicious(request.getCommand())) {
                        ...
                        MessageTranslator.send(VelocityLoader.getServer().getConsoleCommandSource(),
                                Storage.ConfigSections.Settings.PATCH_EXPLOITS.ALERT_MESSAGE.get().replace("%player%",
                                        player.getUsername()));
                        player.disconnect(LegacyComponentSerializer.legacyAmpersand()
                                .deserialize(Storage.ConfigSections.Settings.PATCH_EXPLOITS.KICK_MESSAGE.get()));
        ...
        public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
            ...
            if (packet instanceof PluginMessagePacket pluginMessagePacket) {
                ...
                if (CustomServerBrand.isEnabled() && CustomServerBrand.isBrandTag(pluginMessagePacket.getChannel())
                        && !player.getCurrentServer().isPresent())
                {
                    ...
                    super.write(ctx, new PluginMessagePacket(pluginMessagePacket.getChannel(),
                            brandManipulatePacket.getByteBuf()), promise);
                    return;
                ...
                if (!PermissionUtil.hasBypassPermission(sender) && player.getCurrentServer().isPresent()) {
                    ...
                    String playerInput = getPlayerInput(player), rawPlayerInput = playerInput,
                    ...
                    if (!playerInput.equals("/")) {
                        ...
                        cancelsBeforeHand = !Storage.Blacklist.canPlayerAccessTab(sender, groups,
                                StringUtils.replaceFirst(playerInput, "/", ""), server);

                        if (!cancelsBeforeHand)
                            cancelsBeforeHand = Storage.ConfigSections.Settings.CUSTOM_VERSION
                                    .isTabCompletable(StringUtils.replaceFirst(playerInput, "/", ""))
                                    || Storage.ConfigSections.Settings.CUSTOM_PLUGIN
                                            .isTabCompletable(StringUtils.replaceFirst(playerInput, "/", ""));
                    ...
                    if (cursor.startsWith("/")) {
                        ...
                        if (spaces == 0) {
                            ...
                            response.getOffers().removeIf(offer -> {
                                ...
                                if (Storage.ConfigSections.Settings.CUSTOM_PLUGIN.isTabCompletable(command)
                                        || Storage.ConfigSections.Settings.CUSTOM_VERSION.isTabCompletable(command))
                                {
                            ...
                            FilteredTabCompletionEvent filteredTabCompletionEvent = PATEventHandler
                                    .callFilteredTabCompletionEvents(player.getUniqueId(), rawPlayerInput,
                                            suggestionsAsString);
                            ...
                            response.getOffers().removeIf(
                                    offer -> !filteredTabCompletionEvent.getCompletion().contains(offer.getText()));
                ...
                if (player.getCurrentServer().isPresent()) {
                    ...
                    if (!commands.getRootNode().getChildren().isEmpty()) {
                        ...
                        commands.getRootNode().getChildren().stream()
                        ...
                        final List<String> playerCommands = commandsCache.getPlayerCommands(commandsAsString, sender,
                                groups, serverName);
                        ...
                        commands.getRootNode().getChildren().removeIf(command -> {
                            ...
                            if (Storage.ConfigSections.Settings.CUSTOM_PLUGIN.isTabCompletable(command.getName())
                                    || Storage.ConfigSections.Settings.CUSTOM_VERSION
                                            .isTabCompletable(command.getName()))
                            {
